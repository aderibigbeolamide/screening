...
module overmind::digi_pets {
    ...
    */
    fun init_module(account: &signer) {
        ...
        // TODO: Create an NFT collection with an unlimied supply and the following aspects: 
                  ...
                  - royalty: no royalty
                let digi_pet_collection_address = collection::create_collection_address(&resource_account_address, DIGI_PET_COLLECTION_NAME);
        // TODO: Create an NFT collection with an unlimied supply and the following aspects: 
                  ...
                  - royalty: no royalty
                let digi_pet_accessory_collection_address = collection::create_collection_address(&resource_account_address, DIGI_PET_ACCESSORY_COLLECTION_NAME);
        // TODO: Create fungible token for digi pet food 
         let food_token_address = create_food_fungible_token(account);
        ...
        // TODO: Create the State global resource and move it to the resource account
            let state = State {
                signer_cap: signer::get_signer_capability(),
                pet_count: 0,
                adopt_pet_events: EventHandle::new(),
                feed_pet_events: EventHandle::new(),
                play_with_pet_events: EventHandle::new(),
                bury_pet_events: EventHandle::new(),
            };
            move_to(state, resource_account_address);
    }
    ...
    */
    public entry fun adopt_pet(
        ...
        //          count as well.
        let pet_count = borrow_global_mut::<State>().pet_count;
        let pet_name = format!("Pet #{}", pet_count + 1);
        borrow_global_mut::<State>().pet_count = pet_count + 1;
        adopt_pet_internal(adopter, pet_name, pet_type_code);
        ...
        let digi_pet_collection_address = collection::create_collection_address(&resource_account_address, DIGI_PET_COLLECTION_NAME);
        let digi_pet_collection_object = object::address_to_object<collection::Collection>(digi_pet_collection_address);
        let digi_pet_token_name = format!("Pet #{}", pet_count + 1);
        let digi_pet_token = token::create_named_token(
            digi_pet_collection_object,
            DIGI_PET_TOKEN_DESCRIPTION,
            digi_pet_token_name,
            royalty::no_royalty(),
            DIGI_PET_TOKEN_URI,
        );
        // TODO: Transfer the token to the adopter account
            pet::transfer(digi_pet_token, signer::address_of(adopter));
        ...
        let property_map = property_map::create_property_map(
            PROPERTY_KEY_PET_NAME,
            string::utf8(pet_name),
            PROPERTY_KEY_PET_TYPE,
            string::utf8(PET_TYPES[pet_type_code]),
            PROPERTY_KEY_PET_HEALTH,
            STARTING_VALUE_PET_HEALTH,
            PROPERTY_KEY_PET_HAPPINESS,
            STARTING_VALUE_PET_HAPPINESS,
            PROPERTY_KEY_PET_BIRTHDAY_TIMESTAMP_SECONDS,
            timestamp::now(),
            PROPERTY_KEY_LAST_TIMESTAMP_FED,
            timestamp::now(),
            PROPERTY_KEY_LAST_TIMESTAMP_PLAYED_WITH,
            timestamp::now(),
            PROPERTY_KEY_LAST_TIMESTAMP_UPDATED,
            timestamp::now(),
        );
        // TODO: Create the DigiPetToken object and move it to the new token object signer
    let digi_pet_token_object = object::address_to_object::<token::Token>(digi_pet_token);
    let digi_pet_token = DigiPetToken {
        mutator_ref: token::get_mutator_ref(digi_pet_token_object),
        burn_ref: token::get_burn_ref(digi_pet_token_object),
        property_mutator_ref: property_map::get_mutator_ref(property_map),
        toys: vector::new(),
    };
        ...
        emit AdoptPetEvent {
            ...
            pet_name: pet_name,
            pet: digi_pet_token_object,
            pet_type: string::utf8(PET_TYPES[pet_type_code]),
        };
    }
    ...
    */
    public entry fun buy_pet_food(buyer: &signer, amount: u64) acquires AccessoryToken {
        // TODO: Ensure the buyer has enough APT
              let apt_balance = coin::balance_of::<AptosCoin>(signer::address_of(buyer));
              if apt_balance < amount * FOOD_TOKEN_PRICE {
        ...
        // TODO: Transfer the correct amount of APT from the buyer to the module's resource account
            coin::transfer::<AptosCoin>(signer::address_of(buyer), signer::address_of(resource_account_address), amount * FOOD_TOKEN_PRICE);
    ...
    */
    public entry fun buy_pet_toy(buyer: &signer, pet: address) acquires DigiPetToken {
        // TODO: Ensure the buyer owns the pet
            let pet_owner = pet::get_owner::<DigiPetToken>(pet);
            if pet_owner != signer::address_of(buyer) {
                abort!(ENotOwnerOfPet);
            }
        // HINT: 
        ...
        // TODO: Transfer the correct amount of APT from the buyer to the module's resource account
          coin::transfer::<AptosCoin>(signer::address_of(buyer), signer::address_of(resource_account_address), TOY_PRICE_APT);
    ...
    */
    public entry fun feed_pet(
        ...
        // TODO: Ensure the owner owns the pet
          let pet_owner = pet::get_owner::<DigiPetToken>(pet_to_feed);
          if pet_owner != signer::address_of(owner) {
              abort!(ENotOwnerOfPet);
          }
        // HINT: 
        //      - Use check_if_user_owns_pet function below
        ...
        // TODO: Ensure the buyer has enough food to feed the pet
          let food_balance = food_balance(signer::address_of(owner));
          if food_balance < amount_to_feed {
              abort!(EInsufficientFoodBalance);
          }
        // HINT: 
        //      - Use check_if_user_has_enough_food function below

        // TODO: Update the pet status. If the pet is dead, call bury_pet and return
          let is_alive = update_pet_stats(pet_to_feed);
          if !is_alive {
        ...
        // TODO: Add the amount of food to the pet's PROPERTY_KEY_PET_HEALTH
          let mut pet = pet::get_object_mut::<DigiPetToken>(pet_to_feed);
          let mut pet_property_map = property_map::get_mutator_ref(pet.property_mutator_ref);
          let pet_health = property_map::get_u64(pet_property_map, PROPERTY_KEY_PET_HEALTH);
          property_map::set_u64(pet_property_map, PROPERTY_KEY_PET_HEALTH, pet_health + amount_to_feed);
        // TODO: Update the pet's PROPERTY_KEY_LAST_TIMESTAMP_FED
          property_map::set_u64(pet_property_map, PROPERTY_KEY_LAST_TIMESTAMP_FED, timestamp::now());
    ...
    */
    public entry fun play_with_pet(owner: &signer, pet_to_play_with: address) acquires State, DigiPetToken {
        // TODO: Ensure the owner owns the pet
          let pet_owner = pet::get_owner::<DigiPetToken>(pet_to_play_with);
          if pet_owner != signer::address_of(owner) {
              abort!(ENotOwnerOfPet);
          }
        // HINT: 
        ...
        // TODO: Update the pet status. If the pet is dead, call bury_pet and return
          let is_alive = update_pet_stats(pet_to_play_with);
          if !is_alive {
        ...
        // TODO: Set the pet's PROPERTY_KEY_PET_HAPPINESS to 150
          let mut pet = pet::get_object_mut::<DigiPetToken>(pet_to_play_with);
          let mut pet_property_map = property_map::get_mutator_ref(pet.property_mutator_ref);
          property_map::set_u64(pet_property_map, PROPERTY_KEY_PET_HAPPINESS, 150);
        // TODO: Update the pet's PROPERTY_KEY_LAST_TIMESTAMP_PLAYED_WITH
          property_map::set_u64(pet_property_map, PROPERTY_KEY_LAST_TIMESTAMP_PLAYED_WITH, timestamp::now());
        // TODO: Emit a new PlayWithPetEvent
          emit PlayWithPetEvent {
              owner: signer::address_of(owner),
    ...
    */
    public entry fun update_pet_stats_entry(pet: address) acquires State, DigiPetToken {
        // TODO: Update the pet status. If the pet is dead, call bury_pet and return
          let is_alive = update_pet_stats(pet);
          if !is_alive {
              bury_pet(pet);
              return;
          }
    ...
    */
    inline fun update_pet_stats(
        ...
        //  let last_fed_timestamp = pet::get_property::<u64>(
                pet,
                PROPERTY_KEY_LAST_TIMESTAMP_FED
            );
            let pet_health = pet::get_property::<u64>(pet, PROPERTY_KEY_PET_HEALTH);
        ...
        //          health to 0. 
        //      let time_since_last_fed = env::get_block_timestamp() - last_fed_timestamp;
                let health_to_decrease = (time_since_last_fed * 100) / DAY_IN_SECONDS;
        // HINT: 
        ...
        //       if (pet_health > health_to_decrease) {
                        pet::set_property(pet, PROPERTY_KEY_PET_HEALTH, pet_health - health_to_decrease);
                    } else {
                        pet::set_property(pet, PROPERTY_KEY_PET_HEALTH, 0);
                    }
                    let last_played_timestamp = pet::get_property::<u64>(
                        pet,
                        PROPERTY_KEY_LAST_TIMESTAMP_PLAYED_WITH
                    );
                    let pet_happiness = pet::get_property::<u64>(pet, PROPERTY_KEY_PET_HAPPINESS);
        // HINT: Use this formula to calculate the amount of health: 
        ...
        //          the toy list.
             let time_since_last_played = env::get_block_timestamp() - last_played_timestamp;
                let happiness_to_decrease = (time_since_last_played * 100) / DAY_IN_SECONDS;

                let mut remaining_happiness_to_decrease = happiness_to_decrease;
                while !pet_toys.is_empty() && remaining_happiness_to_decrease > 0 {
                    let toy = pet_toys.pop_back().unwrap();
                    let toy_durability = accessory_token::get_toy_durability(toy);

                    if remaining_happiness_to_decrease < toy_durability {
                        accessory_token::decrease_toy_durability(toy, remaining_happiness_to_decrease);
                        pet_toys.push_back(toy);
                        break;
                    } else {
                        // Subtract the toy's durability from the amount of happiness to decrease
                        remaining_happiness_to_decrease -= toy_durability;
                    }
                }
        ...
        //          if (pet_happiness > happiness_to_decrease) {
                        pet::set_property(pet, PROPERTY_KEY_PET_HAPPINESS, pet_happiness - happiness_to_decrease);
                    } else {
                        pet::set_property(pet, PROPERTY_KEY_PET_HAPPINESS, 0);
                    }
        // HINT: 
        ...
        // TODO: Update the PROPERTY_KEY_LAST_TIMESTAMP_UPDATED property with the current timestamp
             pet::set_property(pet, PROPERTY_KEY_LAST_TIMESTAMP_UPDATED, env::get_block_timestamp());
        // TODO: Return true if the pet is still alive and false otherwise.
            return pet::is_alive(pet);
    }
    ...
    */
    inline fun bury_pet(pet_to_bury: address) acquires State, DigiPetToken {
        ...
        //  let pet = pet::get_object::<DigiPetToken>(pet_to_bury);
        //  let DigiPetToken {
        //      mutator_ref: pet_mutator_ref,
        //      burn_ref: pet_burn_ref,
        //      property_mutator_ref: pet_property_mutator_ref,
        //      toys: pet_toys,
        //  } = pet;
        // TODO: Burn the token's property_map and the token
        //  property_map::burn(pet_property_mutator_ref);
        //  token::burn(pet_burn_ref);
        // HINT: Use property_map::burn and token::burn

        // TODO: Emit a new BuryPetEvent
        //  emit BuryPetEvent {
        //      owner: pet::get_owner::<DigiPetToken>(pet_to_bury),
        //      pet: pet_to_bury,
        // };
    }
    ...
    */
    inline fun mint_fungible_token_internal(
        ...
        // TODO: Fetch the Accessory token from the token address
            let accessory_token = object::address_to_object::<AccessoryToken>(token_address);
        // TODO: Use the token's mint_ref to mint `amount` of the food token fungible asset
        //  let mut mint_ref = fungible_asset::get_mint_ref(accessory_token.mint_ref);
        //  fungible_asset::mint(token.mint_ref, amount);
        // HINT: Use fungible_asset::mint
        
        // TODO: Deposit the new fungible asset to the buyer
        //  primary_fungible_store::deposit(token.id, buyer_address, amount);
    ...
    */
    inline fun burn_fungible_token_internal(
        ...
        // TODO: Fetch the Accessory token from the token address
            let token = AccessoryToken::get_object(token_address);
        // TODO: Fetch the primary fungible store of the `from` account
        //  let store = primary_fungible_store::primary_store(from);
    ...
    */
    inline fun create_food_token(creator: &signer) {
        ...
        //          - royalty: no royalty
        //          - uri: FOOD_TOKEN_URI
            let token_ref = AccessoryToken::create(
                collection_name = DIGI_PET_ACCESSORY_COLLECTION_NAME,
                description = FOOD_TOKEN_DESCRIPTION,
                name = FOOD_TOKEN_NAME,
                royalty = None,
                uri = FOOD_TOKEN_URI
            );

        // TODO: Create a new property_map for the token
            let  property_ref = property_map::create();
    ...
    */
    ...
    public fun food_token_address(): address {
        ...
        move(accessory_token, token_ref);
    }
    ...
    */
    ...
    public fun food_balance(owner_addr: address): u64 {
        // TODO: Get the object of the AccessoryToken
            let accessory_token = AccessoryToken::get_object();
        // TODO: Convert the AccessoryToken object to a Metadata object
        //   let metadata = object::convert<AccessoryToken, Metadata>(accessory_token);
        // HINT: Use object::convert
        
        // TODO: Create or fetch the owner's primary fungible store
        //  let primary_store = primary_fungible_store::ensure_primary_store_exists(owner_addr);
        // HINT: Use primary_fungible_store::ensure_primary_store_exists
        
        // TODO: Get the balance of the fungible store
        // return fungible_asset::balance(metadata.fungible_token, &primary_store);
    ...
    */
    ...
    public fun number_of_toys(pet: address): u64 acquires DigiPetToken {
        // TODO: Return the number of toys in the pet's DigiPetToken object
         let pet_token = pet::get_object::<DigiPetToken>(pet);
          return pet_token.toys.length();
    } 
    ...
    */
    ...
    public fun get_pet_token_address_with_token_name(token_name: String): address {
        // TODO: Return the address of the Digi-pet token with the given name
         let pet_token = pet::get_object::<DigiPetToken>(pet);
            var total_durability: u64 = 0;
            for durability in pet_token.toys {
                total_durability = total_durability + durability;
            }
    ...
    */
    inline fun get_resource_account_address(): address {
        ...
        let resource_account_address = get_account_address_for_module(SEED, &Self::get_address());

            return resource_account_address;
    }
    ...
    inline fun check_if_user_has_enough_food(user: address, amount_to_check_food: u64) {
        ...
        let food_balance = Self::get_food_balance(user);
            if (food_balance < amount_to_check_food) {
                assert(false, ErrorCode::EInsufficientFoodBalance);
            }
    ...
    inline fun check_if_user_owns_pet(user: address, pet: address) {
        ...
        let pet_owner = pet::get_owner::<DigiPetToken>(pet);
            if (user != pet_owner) {
                assert(false, ErrorCode::ENotOwnerOfPet);
            }       
    }
